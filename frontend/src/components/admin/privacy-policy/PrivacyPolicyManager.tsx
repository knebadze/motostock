...
export function PrivacyPolicyManager({ initialPrivacyPolicy }: { initialPrivacyPolicy: PrivacyPolicy }) {
  ...
  const [contentEn, setContentEn] = useState(initialPrivacyPolicy.content.en);
  const [contentRu, setContentRu] = useState(initialPrivacyPolicy.content.ru);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold tracking-tight">კონფიდენციალობის პოლიტიკა</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        ეს ტექსტი გამოჩნდება საიტის საჯარო „კონფიდენციალობის პოლიტიკა” გვერდზე.
      </p>
