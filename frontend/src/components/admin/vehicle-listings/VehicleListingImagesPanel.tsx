...
import {
  deleteVehicleListingImage,
  listVehicleListingImages,
  reorderVehicleListingImages,
  uploadVehicleListingImages,
  type VehicleListingImage,
} from "@/lib/api/vehicle-listing-images";
...
export function VehicleListingImagesPanel({
  listingId,
  onPendingFilesChange,
}: {
  // null while creating a new listing (no id to upload against yet) — images
  // are then held locally and uploaded right after the listing is created.
  listingId: number | null;
  onPendingFilesChange?: (files: File[]) => void;
}) {
  const isAttached = listingId !== null;

  const [remoteImages, setRemoteImages] = useState<VehicleListingImage[]>([]);
  ...
  useEffect(() => {
    ...
    listVehicleListingImages(listingId)
      .then((items) => {
  ...
  async function handleFilesSelected(files: File[]) {
    if (isAttached) {
      setUploading(true);
      try {
        const uploaded = await uploadVehicleListingImages(listingId, files);
  ...
  async function handleConfirmDelete() {
    ...
    await deleteVehicleListingImage(listingId, Number(deletingKey));
  ...
  async function handleThumbDrop(event: React.DragEvent, targetKey: string) {
    ...
    if (isAttached) {
      ...
      try {
        const updated = await reorderVehicleListingImages(
          listingId,
          nextOrder.map(Number),
        );
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs text-muted-foreground">
        ...
        ერთმანეთში დასალაგებლად — პირველი სურათი იქნება მთავარი ფოტო. რეკომენდებული ზომა მინ.
        1200×1200px (კვადრატული).
      </p>
