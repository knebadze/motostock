...
// Same hand-rolled SVG line/area chart as analytics' RevenueTrendChart, just
// for a plain integer visitor count instead of currency — kept as its own
// component rather than a shared generic one since the two differ in axis
// formatting (formatPrice vs a bare integer) and this is the only other
// consumer so far.
export function VisitorTrendChart({ data }: { data: { date: string; visitors: number }[] }) {
  ...
  const max = Math.max(1, ...data.map((row) => row.visitors));
  const xFor = (index: number) =>
    data.length === 1 ? PADDING_LEFT : PADDING_LEFT + (index / (data.length - 1)) * innerWidth;
  const yFor = (visitors: number) => PADDING_TOP + innerHeight - (visitors / max) * innerHeight;

  const linePoints = data.map((row, index) => `${xFor(index)},${yFor(row.visitors)}`).join(" L ");
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6">
      ...
      >
        ...
        <circle cx={xFor(data.length - 1)} cy={yFor(latest.visitors)} r={3} className="fill-primary" />
      ...
      {hoverRow ? (
        <div className="mt-2 flex items-center justify-between text-xs">
          ...
          <span className="font-semibold tabular-nums text-foreground">{hoverRow.visitors}</span>
        </div>
      ) : (
        <div className="mt-2 flex items-center justify-between text-xs">
          <span className="text-muted-foreground">ბოლო დღე — {formatDate(latest.date)}</span>
          <span className="font-semibold tabular-nums text-foreground">{latest.visitors}</span>
        </div>
      )}
    </div>
  );
